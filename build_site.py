...
def baseline_table(data, tail=None, rows=None, na_rows=None):
    ...
    for tag, lbl in [("released", " (published defaults)"), ("matched", " (capacity-matched)")]:
        ...
        # REMIND's Soft-MoE backbone under plain ERM (no robust objective, no residual routing);
        # only run for the no-overlap experiments
        collect(f"SoftMoE_ERM__{tag}", "REMIND backbone with plain ERM", "ext")
    HIGHER = {"worst_acc": True, "tail_acc": True, "wt_acc": True, "wt_f1": True,
              "wt_loss": False, "worst_loss": False, "max_excess": False}
    best, tied = {}, {}
    for col, higher in HIGHER.items():
...
def final_results_page(loaded=None, merged_bl=None):
    ...
    SCAT = {"NHANES": [("fig11_latent_scatter", "Left: no anchors. Middle: class anchors (ours). Right: the randomly-assigned-anchor control from Appendix B of the draft, where each sample is pulled toward a random class's anchor. Top row coloured by group (rings = group centroids), bottom row the same points by outcome; stars are the learnt anchor means and the dashed ellipses the learnt anchor Gaussians at 2 sigma, projected onto the same plane. Between-group distance 2.57 / 0.15 / 0.23: on NHANES any shared target aligns the groups; the class structure is a smaller part. The cloud-to-learnt-anchor W2 (each (group, class) cloud against its anchor) is 0.77 for the real anchors and 0.88 for random ones.")],
            ...
            "EMBED": [("fig11_latent_scatter_embed", "Same three columns, 500 exams per group plotted. Real anchors align the six view groups (1.03 to 0.19); random anchors do not (0.69), so here the class structure is what does the aligning. The clouds sit close to their learnt anchors (cloud-to-anchor W2 0.21 vs 0.73 for random anchors), but the four class anchors themselves sit nearly on top of each other at the base anchor learning rate (5e-5, 20 epochs)."),
                      ("fig11_latent_scatter_embed_main_anchorlr", "Supplementary: the same arm with the anchor parameters given a 10x learning rate (5e-4) in the same optimizer. The four class anchors now separate in density order (anchor-mean separation 0.31 to 1.6 latent-scale units, between-class W2 0.60 to 0.86) while the groups stay aligned (0.24). At 10 seeds this changes no performance number (full method: worst-group acc 62.74 to 62.37, loss 1.066 to 1.077, both n.s.; validation selection does not prefer it), so the reported EMBED protocol keeps the base rate. On EMBED the anchors' benefit is the group alignment, not the class structure.")]}
...
def auroc_block(path, note=None, gcols=("G0 survey", "G1 body + HbA1c/HDL", "G2 BP + lipids")):
    ...
    ref = {sd: min(a for a, _ in g.values()) for sd, g in d["Ours_Regret"].items()}
    body = []
    for key, lab in LAB:
        if key not in d:
            continue
        w = {sd: min(a for a, _ in g.values()) for sd, g in d[key].items()}
        groups = sorted(next(iter(d[key].values())))
        pg = [_st.mean(d[key][sd][g][0] for sd in d[key]) for g in groups]
        acc = _st.mean(min(a for _, a in g.values()) for g in d[key].values()); acc = acc * 100 if acc <= 1 else acc
        p_ = paired_p(ref, w) if key != "Ours_Regret" else None
