...
def traj(g, arm, stride=1):
    W, sel = [], []
    for f in sorted(glob.glob(f"runs/gamma_sweep_fh/g{g}_s{stride}/{arm}_s*_f0/metrics.csv")):
...
fig.suptitle("Fed-Heart: group weight trajectories by step size γ, per-step refresh, 1/G init, eq. 13 floors "
             "(mean of 3 seeds, fold 0). Flat at 0.25 = the max player never engaged.", fontsize=9.5, color=INK, x=.01, ha="left")
fig.tight_layout(rect=(0, 0, 1, .95))
os.makedirs("figs/paper", exist_ok=True)
for ext in ("png", "pdf"):
    fig.savefig(f"figs/paper/fig8_gamma_lambda.{ext}", dpi=170, bbox_inches="tight", facecolor=SURF)
print("  wrote figs/paper/fig8_gamma_lambda.png/.pdf")
