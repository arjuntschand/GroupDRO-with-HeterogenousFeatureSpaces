...
for arm, title in [("groupdro", "GroupDRO"), ("regret_only", "Regret-DRO"), ("ours", "ours: anchors + Regret-DRO")]:
    ...
    for j, g in enumerate(GROUPS):
        a = ax[0, j]
        if "per_group_loss_train" in c[0]:
            a.plot(ep, [e["per_group_loss_train"][g] for e in c], color=COL[j], lw=1.8, ls="--", label="train")
        a.plot(ep, [e["per_group_loss"][g] for e in c], color=COL[j], lw=1.8, ls=":", label="val")
        if "per_group_loss_test" in c[0]:
            a.plot(ep, [e["per_group_loss_test"][g] for e in c], color=COL[j], lw=2.2, label="test")
        a.axhline(RSTAR[g], color="#555", ls="--", lw=1); a.set_title(f"{LABEL[g]}\nR*={RSTAR[g]:.2f}", fontsize=9)
        top = max(max(e.get(k, e["per_group_loss"])[g] for e in c) for k in ("per_group_loss", "per_group_loss_train", "per_group_loss_test"))
        a.grid(alpha=.3); a.set_ylim(0, max(2.6, top * 1.05))
        if j == 0:
            a.legend(fontsize=8, frameon=False)
        b = ax[1, j]; b.plot(ep, [e["lambda"][j] for e in c], color=COL[j], lw=2); b.set_ylim(0, 1.02); b.grid(alpha=.3); b.set_xlabel("epoch")
    ax[0, 0].set_ylabel("loss"); ax[1, 0].set_ylabel("group weight λ_g")
    fig.suptitle(f"EMBED — {title}: per-group loss (train dashed, val dotted, test solid) and group weight, seed 0 (uniform init, γ 2.0, train-signal schedule)", fontsize=11)
    fig.tight_layout()
    for ext in ("png", "pdf"):
