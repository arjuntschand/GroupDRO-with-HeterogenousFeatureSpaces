...
def train(cfg):
    ...
    train_loader, test_loader, dataset_info = build_fedheart_loaders(
        ...
        fold_index=cfg.get("fold_index", 0),
    )
    ...
    for epoch in range(1, cfg["epochs"] + 1):
        ...
        if val_metrics is None:
            ...
            _sel = val_metrics
        if _sel["worst_group_acc"] > best_worst_group_acc:
            best_worst_group_acc = _sel["worst_group_acc"]
            best_epoch = epoch
        ...
        if lr_scheduler is not None:
            if cfg.get("lr_scheduler") == "cosine":
                ...
                lr_scheduler.step(_sel["worst_group_acc"])
