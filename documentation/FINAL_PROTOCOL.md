...
| Reweigh / Flex-MoE / REMIND | 72.4 / 72.4 / 73.0 \| 0.64 / 0.79 / 0.62 | 72.8 / 72.7 / 72.5 \| 0.68 / 0.61 / 0.65 | 59.3 / 62.3 / 61.8 \| 2.12 / 1.33 / 1.21 |

Paired over seeds: full method vs common-features ERM, Fed-Heart +3.96 (p < 0.001), NHANES +5.39 (p = 0.001), EMBED +0.58 (p = 0.68) and loss −0.28 (p < 0.001). Against the three published baselines the full method wins on worst-group loss and on regret 9/9 on the mean; 8/9 significant for each, the exception being Fed-Heart vs REMIND (loss 7.3% lower, p = 0.053; regret 17.8% lower, p = 0.066). Where significant, worst-group loss is 11–50% lower and regret 21–80% lower. Worst-group accuracy ties 9/9; overall accuracy ties on the tabular datasets and is significantly higher than all three baselines on EMBED. Baselines: Reweigh is inverse-frequency group reweighting on the Soft MoE backbone (the REMIND paper's column of that name is logit adjustment); Flex-MoE follows its released architecture without noisy gating, routing loss or load balancing; REMIND is a reimplementation with the paper's group-specific residual routing (entropy-gated, second-stage) and its exponentiated weight update (γ 0.02, every 50 steps), corrected on 2026-09-18; the earlier version without residual routing is archived under `runs/baselines_*_softmoe_gdro`. Parameter counts are inference parameters (anchors excluded). Regret-DRO vs GroupDRO is a tie on accuracy everywhere. Anchors: NHANES +3.83 over Regret-DRO (p = 0.004); Fed-Heart −0.94 (n.s.) and −1.84 vs GroupDRO (p = 0.03); EMBED +0.02 loss vs GroupDRO (n.s.).
