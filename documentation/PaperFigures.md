...
| # | Where | Job | Status |
|---|---|---|---|
| 1 | Introduction (page 1-2) | Make the problem visible before any notation: patients with different tests, the two standard fixes failing, and loss vs regret in one picture | not started (bottom row of the current draft is its core) |
| 2 | Setup, next to eq. 5-6 and eq. 12 | Map the notation onto a picture: encoders, shared latent space with anchors, one head, the regret-driven weight update | draft v1: `figs/paper/fig2_method.{svg,pdf,png}` |
